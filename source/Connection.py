...
class Connection(Entity):
    ...
    def export_json(self, data: dict, scenario_name: str):
        ...
        data = self.link_nodes(data, scenario_name)
        data = add_entity(data, "connection", self.full_name, scenario_name)
        for key, values in self.direct_parameters.items():
            ...
            if not key.startswith("NaM") and "(" not in key:
                data = add_parameter_value(data, "connection", self.full_name, key, values["value"], values["type"], scenario_name)
        return data

    def link_nodes(self, data: dict, scenario_name: str):
        # Link the connection to its source and destination nodes in the data dictionary
        data = add_entity(data, "connection__from_node", [self.full_name, self.node_from.full_name], scenario_name)
        data = add_entity(data, "connection__to_node", [self.full_name, self.node_to.full_name], scenario_name)
        data = add_entity(data, "connection__node__node", [self.full_name, self.node_to.full_name, self.node_from.full_name], scenario_name)
        ...
        for parameter in self.direct_parameters:
            ...
            if "(from_node)" in parameter:
                ...
                data = add_parameter_value(data, "connection__from_node", [self.full_name, self.node_from.full_name], parameter_name, self.direct_parameters[parameter]["value"], self.direct_parameters[parameter]["type"], scenario_name)
            if "(to_node)" in parameter:
                # Handle parameters specific to the destination node
                parameter_name = parameter.split("(")[0]
                data = add_parameter_value(data, "connection__to_node", [self.full_name, self.node_to.full_name], parameter_name, self.direct_parameters[parameter]["value"], self.direct_parameters[parameter]["type"], scenario_name)
            if "(from_node_to_node)" in parameter:
                # Handle parameters specific to the relationship between the source and destination nodes
                parameter_name = parameter.split("(")[0]
                data = add_parameter_value(data, "connection__node__node", [self.full_name, self.node_to.full_name, self.node_from.full_name], parameter_name, self.direct_parameters[parameter]["value"], self.direct_parameters[parameter]["type"], scenario_name)
        return data